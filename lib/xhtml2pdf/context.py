...
from util import getSize, getCoords, getFile, pisaFileObject, getFrameDimensions, getColor
from w3c import css as css
...
class pisaCSSBuilder(css.CSSBuilder):
    ...
    def atPage(self, name, pseudopage, declarations):
        ...
        if "-pdf-page-size" in data:
            c.pageSize = default.PML_PAGESIZES.get(str(data["-pdf-page-size"]).lower(), c.pageSize)
        ...
        if "size" in data:
            ...
            for value in size:
                ...
                if type(value) is types.TupleType:
                    ...
                    isLandscape = True
                elif valueStr in default.PML_PAGESIZES:
                    c.pageSize = default.PML_PAGESIZES[valueStr]
                else:
...
class pisaContext(object):
    ...
    def warning(self, msg, *args):
        self.warn += 1
        self.log.append((default.PML_WARNING, self._getLineNumber(), str(msg), self._getFragment(50)))
        try:
    ...
    def error(self, msg, *args):
        self.err += 1
        self.log.append((default.PML_ERROR, self._getLineNumber(), str(msg), self._getFragment(50)))
        try:
            return self.context(msg % args)
        except:
            return self.context(msg)
